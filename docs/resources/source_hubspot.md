...
- `definition_id` (String) The UUID of the connector definition. One of configuration.sourceType or definitionId must be provided. Default: "36c891d9-4bd9-43ac-bad2-10e12756272c"; Requires replacement if changed.
...
- `lookback_window` (Number) How far back (in minutes) to re-fetch records during incremental syncs.  Set this if you notice missing records in CRMSearch streams to recover data  that may be delayed by the HubSpot API. Default: 0
- `num_worker` (Number) The number of worker threads to use for the sync. Default: 10
