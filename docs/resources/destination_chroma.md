...
page_title: "airbyte_destination_chroma Resource - terraform-provider-airbyte"
subcategory: ""
description: |-
  DestinationChroma Resource
---

# airbyte_destination_chroma (Resource)
...
- `chroma_default_embedding_function` (Attributes) Do not calculate embeddings. Chromadb uses the sentence transfomer (https://www.sbert.net/index.html) as a default if an embedding function is not defined. Note that depending on your hardware, calculating embeddings locally can be very slow and is mostly suited for prototypes. (see [below for nested schema](#nestedatt--configuration--embedding--chroma_default_embedding_function))
...
<a id="nestedatt--configuration--embedding--chroma_default_embedding_function"></a>
### Nested Schema for `configuration.embedding.chroma_default_embedding_function`
...
- `auth_method` (Attributes) Mode how to connect to Chroma (see [below for nested schema](#nestedatt--configuration--indexing--auth_method))
- `collection_name` (String) The collection to load data into
...
- `client_server_mode` (Attributes) Authenticate using username and password (suitable for self-managed Chroma clusters) (see [below for nested schema](#nestedatt--configuration--indexing--auth_method--client_server_mode))
- `persistent_client_mode` (Attributes) Configure Chroma to save and load from your local machine (see [below for nested schema](#nestedatt--configuration--indexing--auth_method--persistent_client_mode))

<a id="nestedatt--configuration--indexing--auth_method--client_server_mode"></a>
### Nested Schema for `configuration.indexing.auth_method.client_server_mode`

Required:

- `host` (String) The URL to the chromadb instance
- `port` (Number) The port to the chromadb instance
- `ssl` (Boolean) Whether to use SSL to connect to the Chroma server

Optional:

- `password` (String, Sensitive) Password used in server/client mode only. Default: ""
- `username` (String) Username used in server/client mode only. Default: ""


<a id="nestedatt--configuration--indexing--auth_method--persistent_client_mode"></a>
### Nested Schema for `configuration.indexing.auth_method.persistent_client_mode`

Required:

- `path` (String) Where Chroma will store its database files on disk, and load them on start.
...
```terraform
import {
  to = airbyte_destination_chroma.my_airbyte_destination_chroma
  id = "..."
}
```
