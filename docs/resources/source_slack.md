...
- `definition_id` (String) The UUID of the connector definition. One of configuration.sourceType or definitionId must be provided. Default: "c2281cee-86f9-4a86-bb48-d23286b4c7bd"; Requires replacement if changed.
...
- `channel_filter` (List of String) A channel name list (without leading '#' char) which limit the channels from which you'd like to sync. Empty list means no filter. Default: []
- `channel_messages_window_size` (Number) The size (in days) of the date window that will be used while syncing data from the channel messages stream. A smaller window will allow for greater parallelization when syncing records, but can lead to rate limiting errors. Default: 100
...
- `num_workers` (Number) The number of worker threads to use for the sync. Default: 2
