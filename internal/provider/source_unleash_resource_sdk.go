// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package provider

import (
	"github.com/airbytehq/terraform-provider-airbyte/internal/sdk/models/shared"
	"github.com/hashicorp/terraform-plugin-framework/types"
)

func (r *SourceUnleashResourceModel) ToSharedSourceUnleashCreateRequest() *shared.SourceUnleashCreateRequest {
	var name string
	name = r.Name.ValueString()

	definitionID := new(string)
	if !r.DefinitionID.IsUnknown() && !r.DefinitionID.IsNull() {
		*definitionID = r.DefinitionID.ValueString()
	} else {
		definitionID = nil
	}
	var workspaceID string
	workspaceID = r.WorkspaceID.ValueString()

	var apiToken string
	apiToken = r.Configuration.APIToken.ValueString()

	var apiURL string
	apiURL = r.Configuration.APIURL.ValueString()

	projectName := new(string)
	if !r.Configuration.ProjectName.IsUnknown() && !r.Configuration.ProjectName.IsNull() {
		*projectName = r.Configuration.ProjectName.ValueString()
	} else {
		projectName = nil
	}
	nameprefix := new(string)
	if !r.Configuration.Nameprefix.IsUnknown() && !r.Configuration.Nameprefix.IsNull() {
		*nameprefix = r.Configuration.Nameprefix.ValueString()
	} else {
		nameprefix = nil
	}
	configuration := shared.SourceUnleash{
		APIToken:    apiToken,
		APIURL:      apiURL,
		ProjectName: projectName,
		Nameprefix:  nameprefix,
	}
	secretID := new(string)
	if !r.SecretID.IsUnknown() && !r.SecretID.IsNull() {
		*secretID = r.SecretID.ValueString()
	} else {
		secretID = nil
	}
	out := shared.SourceUnleashCreateRequest{
		Name:          name,
		DefinitionID:  definitionID,
		WorkspaceID:   workspaceID,
		Configuration: configuration,
		SecretID:      secretID,
	}
	return &out
}

func (r *SourceUnleashResourceModel) RefreshFromSharedSourceResponse(resp *shared.SourceResponse) {
	if resp != nil {
		r.CreatedAt = types.Int64Value(resp.CreatedAt)
		r.DefinitionID = types.StringValue(resp.DefinitionID)
		r.Name = types.StringValue(resp.Name)
		r.SourceID = types.StringValue(resp.SourceID)
		r.SourceType = types.StringValue(resp.SourceType)
		r.WorkspaceID = types.StringValue(resp.WorkspaceID)
	}
}

func (r *SourceUnleashResourceModel) ToSharedSourceUnleashPutRequest() *shared.SourceUnleashPutRequest {
	var name string
	name = r.Name.ValueString()

	var workspaceID string
	workspaceID = r.WorkspaceID.ValueString()

	var apiToken string
	apiToken = r.Configuration.APIToken.ValueString()

	var apiURL string
	apiURL = r.Configuration.APIURL.ValueString()

	projectName := new(string)
	if !r.Configuration.ProjectName.IsUnknown() && !r.Configuration.ProjectName.IsNull() {
		*projectName = r.Configuration.ProjectName.ValueString()
	} else {
		projectName = nil
	}
	nameprefix := new(string)
	if !r.Configuration.Nameprefix.IsUnknown() && !r.Configuration.Nameprefix.IsNull() {
		*nameprefix = r.Configuration.Nameprefix.ValueString()
	} else {
		nameprefix = nil
	}
	configuration := shared.SourceUnleashUpdate{
		APIToken:    apiToken,
		APIURL:      apiURL,
		ProjectName: projectName,
		Nameprefix:  nameprefix,
	}
	out := shared.SourceUnleashPutRequest{
		Name:          name,
		WorkspaceID:   workspaceID,
		Configuration: configuration,
	}
	return &out
}
