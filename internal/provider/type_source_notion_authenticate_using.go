// Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.

package provider

type SourceNotionAuthenticateUsing struct {
	SourceNotionAuthenticateUsingAccessToken       *DestinationMilvusIndexingAuthenticationAPIToken `tfsdk:"source_notion_authenticate_using_access_token"`
	SourceNotionAuthenticateUsingOAuth20           *SourceNotionAuthenticateUsingOAuth20            `tfsdk:"source_notion_authenticate_using_o_auth2_0"`
	SourceNotionUpdateAuthenticateUsingAccessToken *DestinationMilvusIndexingAuthenticationAPIToken `tfsdk:"source_notion_update_authenticate_using_access_token"`
	SourceNotionUpdateAuthenticateUsingOAuth20     *SourceNotionAuthenticateUsingOAuth20            `tfsdk:"source_notion_update_authenticate_using_o_auth2_0"`
}
