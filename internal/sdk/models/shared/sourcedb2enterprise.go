...
func CreateEncryptionUnencrypted(unencrypted Unencrypted) Encryption {
	typ := EncryptionTypeUnencrypted
...
func (u *Encryption) UnmarshalJSON(data []byte) error {
	...
	if err := utils.UnmarshalJSON(data, &unencrypted, "", true, nil); err == nil {
		candidates = append(candidates, utils.UnionCandidate{
			Type:  EncryptionTypeUnencrypted,
			Value: &unencrypted,
		})
	}
	...
	if err := utils.UnmarshalJSON(data, &tlsEncryptedVerifyCertificate, "", true, nil); err == nil {
		candidates = append(candidates, utils.UnionCandidate{
			Type:  EncryptionTypeTLSEncryptedVerifyCertificate,
	...
	// Set the union type and value based on the best candidate
	u.Type = best.Type.(EncryptionType)
	switch best.Type {
	case EncryptionTypeUnencrypted:
		u.Unencrypted = best.Value.(*Unencrypted)
		return nil
	case EncryptionTypeTLSEncryptedVerifyCertificate:
...
func (s *SourceDb2Enterprise) GetEncryption() Encryption {
	if s == nil {
		return Encryption{}
	}
	return s.Encryption
