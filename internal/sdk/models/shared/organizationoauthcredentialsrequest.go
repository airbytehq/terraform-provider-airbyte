// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package shared

// OrganizationOAuthCredentialsRequest - POST body for creating/updating organization level OAuth credentials
type OrganizationOAuthCredentialsRequest struct {
	// Whether you're setting this override for a source or destination
	ActorType ActorTypeEnum `json:"actorType"`
	// The name of the source i.e. google-ads
	Name string `json:"name"`
	// The configuration for this source/destination based on the OAuth section of the relevant specification.
	Configuration OAuthCredentialsConfiguration `json:"configuration"`
}

func (o *OrganizationOAuthCredentialsRequest) GetActorType() ActorTypeEnum {
	if o == nil {
		return ActorTypeEnum("")
	}
	return o.ActorType
}

func (o *OrganizationOAuthCredentialsRequest) GetName() string {
	if o == nil {
		return ""
	}
	return o.Name
}

func (o *OrganizationOAuthCredentialsRequest) GetConfiguration() OAuthCredentialsConfiguration {
	if o == nil {
		return OAuthCredentialsConfiguration{}
	}
	return o.Configuration
}
