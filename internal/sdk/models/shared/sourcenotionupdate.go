// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package shared

import (
	"encoding/json"
	"errors"
	"fmt"
	"github.com/airbytehq/terraform-provider-airbyte/internal/sdk/internal/utils"
	"github.com/airbytehq/terraform-provider-airbyte/internal/sdk/types"
	"time"
)

type SourceNotionUpdateAccessToken struct {
	//lint:ignore U1000 accessed via reflection for JSON marshaling
	authType *string `const:"token" json:"auth_type,omitempty"`
	// The Access Token for your private Notion integration. See the <a href='https://docs.airbyte.com/integrations/sources/notion#step-1-create-an-integration-in-notion'>docs</a> for more information on how to obtain this token.
	Token *string `json:"token,omitempty"`
}

func (s SourceNotionUpdateAccessToken) MarshalJSON() ([]byte, error) {
	return utils.MarshalJSON(s, "", false)
}

func (s *SourceNotionUpdateAccessToken) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &s, "", false, nil); err != nil {
		return err
	}
	return nil
}

func (s *SourceNotionUpdateAccessToken) GetAuthType() *string {
	return types.Pointer("token")
}

func (s *SourceNotionUpdateAccessToken) GetToken() *string {
	if s == nil {
		return nil
	}
	return s.Token
}

type SourceNotionUpdateOAuth20 struct {
	// The Access Token received by completing the OAuth flow for your Notion integration. See our <a href='https://docs.airbyte.com/integrations/sources/notion#step-2-set-permissions-and-acquire-authorization-credentials'>docs</a> for more information.
	AccessToken *string `json:"access_token,omitempty"`
	//lint:ignore U1000 accessed via reflection for JSON marshaling
	authType *string `const:"OAuth2.0" json:"auth_type,omitempty"`
	// The Client ID of your Notion integration. See our <a href='https://docs.airbyte.com/integrations/sources/notion#step-2-set-permissions-and-acquire-authorization-credentials'>docs</a> for more information.
	ClientID *string `json:"client_id,omitempty"`
	// The Client Secret of your Notion integration. See our <a href='https://docs.airbyte.com/integrations/sources/notion#step-2-set-permissions-and-acquire-authorization-credentials'>docs</a> for more information.
	ClientSecret *string `json:"client_secret,omitempty"`
}

func (s SourceNotionUpdateOAuth20) MarshalJSON() ([]byte, error) {
	return utils.MarshalJSON(s, "", false)
}

func (s *SourceNotionUpdateOAuth20) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &s, "", false, nil); err != nil {
		return err
	}
	return nil
}

func (s *SourceNotionUpdateOAuth20) GetAccessToken() *string {
	if s == nil {
		return nil
	}
	return s.AccessToken
}

func (s *SourceNotionUpdateOAuth20) GetAuthType() *string {
	return types.Pointer("OAuth2.0")
}

func (s *SourceNotionUpdateOAuth20) GetClientID() *string {
	if s == nil {
		return nil
	}
	return s.ClientID
}

func (s *SourceNotionUpdateOAuth20) GetClientSecret() *string {
	if s == nil {
		return nil
	}
	return s.ClientSecret
}

// #region class-body-sourcenotionupdateoauth20
// #endregion class-body-sourcenotionupdateoauth20

type SourceNotionUpdateAuthenticationMethodType string

const (
	SourceNotionUpdateAuthenticationMethodTypeSourceNotionUpdateOAuth20     SourceNotionUpdateAuthenticationMethodType = "source-notion-update_OAuth2.0"
	SourceNotionUpdateAuthenticationMethodTypeSourceNotionUpdateAccessToken SourceNotionUpdateAuthenticationMethodType = "source-notion-update_Access Token"
)

// SourceNotionUpdateAuthenticationMethod - Choose either OAuth (recommended for Airbyte Cloud) or Access Token. See our <a href='https://docs.airbyte.com/integrations/sources/notion#setup-guide'>docs</a> for more information.
type SourceNotionUpdateAuthenticationMethod struct {
	SourceNotionUpdateOAuth20     *SourceNotionUpdateOAuth20     `queryParam:"inline" union:"member"`
	SourceNotionUpdateAccessToken *SourceNotionUpdateAccessToken `queryParam:"inline" union:"member"`

	Type SourceNotionUpdateAuthenticationMethodType
}

func CreateSourceNotionUpdateAuthenticationMethodSourceNotionUpdateOAuth20(sourceNotionUpdateOAuth20 SourceNotionUpdateOAuth20) SourceNotionUpdateAuthenticationMethod {
	typ := SourceNotionUpdateAuthenticationMethodTypeSourceNotionUpdateOAuth20

	return SourceNotionUpdateAuthenticationMethod{
		SourceNotionUpdateOAuth20: &sourceNotionUpdateOAuth20,
		Type:                      typ,
	}
}

func CreateSourceNotionUpdateAuthenticationMethodSourceNotionUpdateAccessToken(sourceNotionUpdateAccessToken SourceNotionUpdateAccessToken) SourceNotionUpdateAuthenticationMethod {
	typ := SourceNotionUpdateAuthenticationMethodTypeSourceNotionUpdateAccessToken

	return SourceNotionUpdateAuthenticationMethod{
		SourceNotionUpdateAccessToken: &sourceNotionUpdateAccessToken,
		Type:                          typ,
	}
}

func (u *SourceNotionUpdateAuthenticationMethod) UnmarshalJSON(data []byte) error {

	var candidates []utils.UnionCandidate

	// Collect all valid candidates
	var sourceNotionUpdateOAuth20 SourceNotionUpdateOAuth20 = SourceNotionUpdateOAuth20{}
	if err := utils.UnmarshalJSON(data, &sourceNotionUpdateOAuth20, "", true, nil); err == nil {
		candidates = append(candidates, utils.UnionCandidate{
			Type:  SourceNotionUpdateAuthenticationMethodTypeSourceNotionUpdateOAuth20,
			Value: &sourceNotionUpdateOAuth20,
		})
	}

	var sourceNotionUpdateAccessToken SourceNotionUpdateAccessToken = SourceNotionUpdateAccessToken{}
	if err := utils.UnmarshalJSON(data, &sourceNotionUpdateAccessToken, "", true, nil); err == nil {
		candidates = append(candidates, utils.UnionCandidate{
			Type:  SourceNotionUpdateAuthenticationMethodTypeSourceNotionUpdateAccessToken,
			Value: &sourceNotionUpdateAccessToken,
		})
	}

	if len(candidates) == 0 {
		return fmt.Errorf("could not unmarshal `%s` into any supported union types for SourceNotionUpdateAuthenticationMethod", string(data))
	}

	// Pick the best candidate using multi-stage filtering
	best := utils.PickBestUnionCandidate(candidates, data)
	if best == nil {
		return fmt.Errorf("could not unmarshal `%s` into any supported union types for SourceNotionUpdateAuthenticationMethod", string(data))
	}

	// Set the union type and value based on the best candidate
	u.Type = best.Type.(SourceNotionUpdateAuthenticationMethodType)
	switch best.Type {
	case SourceNotionUpdateAuthenticationMethodTypeSourceNotionUpdateOAuth20:
		u.SourceNotionUpdateOAuth20 = best.Value.(*SourceNotionUpdateOAuth20)
		return nil
	case SourceNotionUpdateAuthenticationMethodTypeSourceNotionUpdateAccessToken:
		u.SourceNotionUpdateAccessToken = best.Value.(*SourceNotionUpdateAccessToken)
		return nil
	}

	return fmt.Errorf("could not unmarshal `%s` into any supported union types for SourceNotionUpdateAuthenticationMethod", string(data))
}

func (u SourceNotionUpdateAuthenticationMethod) MarshalJSON() ([]byte, error) {
	if u.SourceNotionUpdateOAuth20 != nil {
		return utils.MarshalJSON(u.SourceNotionUpdateOAuth20, "", true)
	}

	if u.SourceNotionUpdateAccessToken != nil {
		return utils.MarshalJSON(u.SourceNotionUpdateAccessToken, "", true)
	}

	return nil, errors.New("could not marshal union type SourceNotionUpdateAuthenticationMethod: all fields are null")
}

type SourceNotionUpdateSourceType string

const (
	SourceNotionUpdateSourceTypeNotion SourceNotionUpdateSourceType = "notion"
)

func (e SourceNotionUpdateSourceType) ToPointer() *SourceNotionUpdateSourceType {
	return &e
}
func (e *SourceNotionUpdateSourceType) UnmarshalJSON(data []byte) error {
	var v string
	if err := json.Unmarshal(data, &v); err != nil {
		return err
	}
	switch v {
	case "notion":
		*e = SourceNotionUpdateSourceType(v)
		return nil
	default:
		return fmt.Errorf("invalid value for SourceNotionUpdateSourceType: %v", v)
	}
}

type SourceNotionUpdate struct {
	// Choose either OAuth (recommended for Airbyte Cloud) or Access Token. See our <a href='https://docs.airbyte.com/integrations/sources/notion#setup-guide'>docs</a> for more information.
	Credentials *SourceNotionUpdateAuthenticationMethod `json:"credentials,omitempty"`
	// UTC date and time in the format YYYY-MM-DDTHH:MM:SS.000Z. During incremental sync, any data generated before this date will not be replicated. If left blank, the start date will be set to 2 years before the present date.
	StartDate *time.Time `json:"start_date,omitempty"`
	//lint:ignore U1000 accessed via reflection for JSON marshaling
	sourceType *SourceNotionUpdateSourceType `const:"notion" json:"sourceType"`
}

func (s SourceNotionUpdate) MarshalJSON() ([]byte, error) {
	return utils.MarshalJSON(s, "", false)
}

func (s *SourceNotionUpdate) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &s, "", false, nil); err != nil {
		return err
	}
	return nil
}

func (s *SourceNotionUpdate) GetCredentials() *SourceNotionUpdateAuthenticationMethod {
	if s == nil {
		return nil
	}
	return s.Credentials
}

func (s *SourceNotionUpdate) GetStartDate() *time.Time {
	if s == nil {
		return nil
	}
	return s.StartDate
}

func (s *SourceNotionUpdate) GetSourceType() *SourceNotionUpdateSourceType {
	return SourceNotionUpdateSourceTypeNotion.ToPointer()
}
